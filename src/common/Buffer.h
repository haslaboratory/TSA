...
template <typename T> class BufferViewT {
public:
    using Type = T;
...
public:
    ...
    // This moves the data pointer forwards so that the first amount bytes are
    // no longer considered valid data.  The caller is responsible for ensuring
    // that amount is less than or equal to the actual data length.
    //
    ...
    void resize(size_t size) {
        assert(size >= size_);
        size_ = size;
        data_.reset(allocate(size));
    }

private:
    Buffer copyInternal(Buffer buf) const {
        if (data_) {
            assert(buf.data_ != nullptr);
    ...
    static uint8_t *allocate(size_t size, size_t alignment) {
        // assert(folly::isPowTwo(alignment)); // Also ensures @alignment > 0
        assert(size % alignment == 0u);
        auto ptr =
            reinterpret_cast<uint8_t *>(::aligned_alloc(alignment, size));
        if (!ptr) {
            throw std::bad_alloc();
        }
        return ptr;
    ...
    // size_ represents the size of valid data in the data_, i.e., "size_"
    // number of bytes from startOffset in data_ are considered valid in the
    // Buffer
    size_t size_{};
